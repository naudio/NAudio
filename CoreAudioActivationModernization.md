...
5. **Callback interfaces are out of scope for Phase 2e.** `IMMNotificationClient`, `IControlChangeNotify`, `IAudioEndpointVolumeCallback`, `IAudioSessionNotification`, `IAudioSessionEvents`, `IActivateAudioInterfaceCompletionHandler`, `IAgileObject` stay on `[ComImport]` and the legacy CCW path until Phase 2f.
...
- The seven remaining `[ComImport]` callback interfaces in `CoreAudioApi/Interfaces/` — Phase 2f.
