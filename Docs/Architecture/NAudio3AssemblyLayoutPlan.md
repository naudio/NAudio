...
4. **`NAudio.Wasapi` is misnamed.** It contains four distinct Windows APIs — WASAPI/CoreAudio, Media Foundation, DMO, and DirectSound — each with its own SDK history, deprecation curve, and audience. *(Acknowledged but **deferred** beyond the first round — see §"Subsequent phases".)* It also once carried the WinRT MIDI shim, which was only parked there for TFM convenience; the second round moved it to `NAudio.Midi` (see §"Execution status — second round" and #1384).

## Design principles

- **Cross-platform code lives in `net9.0` assemblies; OS-specific code is isolated in its own assembly and marked `[SupportedOSPlatform("<os>")]`.** The TFM only carries an OS suffix (`net9.0-windows`, and a version like `…10.0.19041.0`) when the code needs OS-SDK *reference assemblies* — i.e. WinRT/WinForms/WPF projections. Windows-only code that is pure COM interop or P/Invoke stays on plain `net9.0` + `[SupportedOSPlatform("windows")]` (this is how `NAudio.Wasapi` is targeted after the second round, and mirrors how the Linux/mac backends use plain `net9.0` + `[SupportedOSPlatform("linux"/"macos")]`). No `PlatformNotSupportedException`-at-runtime surprises either way.
...
- **The meta-package multi-targets** so that a Windows consumer auto-receives the full Windows audio stack and a `net9.0` consumer receives only the portable subset. This is how NuGet is designed to handle the OS split, and it works without RID-conditional restore tricks (which NuGet doesn't really support for managed-package selection anyway). The meta-package's Windows leg uses `net9.0-windows10.0.19041.0` because that is what exposes the WinRT MIDI projections in `NAudio.Midi`'s Windows leg — see §"Meta-package" for the rationale.
...
- **WinRT MIDI landed by dual-targeting `NAudio.Midi`** (second round, #1384) rather than as a separate `NAudio.Midi.WinRT` package. `NAudio.Midi` now targets `net9.0;net9.0-windows10.0.19041.0`: the portable leg is unchanged, and the Windows leg adds `WinRTMidiIn`/`WinRTMidiOut`/`MidiMessageConverter` (`Windows.Devices.Midi`). This keeps WinRT MIDI in its natural home (it already lived in namespace `NAudio.Midi`, implementing the `IMidiInput`/`IMidiOutput` interfaces defined here) without a third package. Trade-off: `NAudio.Midi` is no longer a single-TFM assembly and sets `EnableWindowsTargeting=true` so its *source* still builds on Linux/macOS; NuGet consumers resolve the `net9.0` asset and are unaffected. The separate-package option (keep `NAudio.Midi` pure `net9.0`, ship WinRT MIDI alongside) was considered and set aside as more packaging overhead for equivalent capability.
...
| `NAudio.Midi` | MIDI message types, `MidiFile` reader/writer, `MidiEvent` hierarchy (portable). **Windows leg only:** `WinRTMidiIn`/`WinRTMidiOut`/`MidiMessageConverter` live MIDI I/O over `Windows.Devices.Midi` | Portable `net9.0` leg as before (`MidiIn`/`MidiOut` moved out to `NAudio.WinMM`; `<IsAotCompatible>true</IsAotCompatible>`). Second round: became dual-target (`net9.0;net9.0-windows10.0.19041.0`) to host WinRT MIDI on the Windows leg — see §"Design principles" and #1384. The portable leg omits the WinRT files, so cross-platform consumers are unchanged. |

### Windows-specific (`[SupportedOSPlatform("windows")]`; `net9.0-windows*` unless noted)

*(Most Windows backends target `net9.0-windows`. `NAudio.Wasapi` is the exception — plain `net9.0` since the second round, because it has no WinRT/WinForms/WPF dependency; see its row and §"Design principles".)*

| Package | Contents | Notes |
|---|---|---|
| `NAudio.Wasapi` | WASAPI/CoreAudio (`AudioClient`, `MMDevice`, `MMDeviceEnumerator`, `WasapiCapture`, `WasapiLoopbackCapture`, `WasapiOut`, `WasapiPlayer`, `WasapiRecorder`), Media Foundation (`MediaFoundationReader`, `StreamMediaFoundationReader`, `MediaFoundationEncoder`, `MediaFoundationResampler`) | Scope narrowed in the second round (see §"Execution status — second round"): DMO and DirectSound carved out into `NAudio.Dmo`, and the WinRT MIDI shim moved to `NAudio.Midi`. What remains is WASAPI + MF — all COM interop / P/Invoke. **Targets plain `net9.0`** (not `net9.0-windows*`): with no WinRT dependency left, the Windows-only guarantee is carried entirely by the assembly-level `[SupportedOSPlatform("windows")]`, so cross-platform apps can reference it and build off-Windows without `EnableWindowsTargeting` (#1384). Process-loopback members carry a narrower `[SupportedOSPlatform("windows10.0.19041.0")]`. |
...
This layout is what let WinRT MIDI land cleanly in the second round: portable `NAudio.Midi` already defined the shared `IMidiInput` / `IMidiOutput` interfaces (the winmm `MidiIn` / `MidiOut` in `NAudio.WinMM` implement them), so `WinRTMidiIn` / `WinRTMidiOut` — which already lived in namespace `NAudio.Midi` — simply moved back home. Rather than a separate `NAudio.Midi.WinRT` package, `NAudio.Midi` gained a `net9.0-windows10.0.19041.0` leg (which exposes the WinRT MIDI projections) alongside its portable `net9.0` leg; see §"Design principles" and #1384.
...
| Package | Target | Contents |
|---|---|---|
| `NAudio` | `net9.0;net9.0-windows;net9.0-windows10.0.19041.0` (multi-targeted) | TFM-conditional `PackageReference` set: `net9.0` pulls Core + Midi (portable leg) only; **both** Windows legs pull the full win32 stack (Core + Midi + Asio + Wasapi + WinMM + WinForms — note `Wasapi` still bundles MediaFoundation, and `WinMM` brings legacy `MidiIn`/`MidiOut` along). Mirrors the v2 meta-package shape so existing Windows consumers don't need to add `PackageReference`s on upgrade. WinRT MIDI reaches consumers who target `net9.0-windows10.0.19041.0` or later automatically via `NAudio.Midi`'s Windows leg — no separate package or reference needed. |

**Why both `net9.0-windows` and `net9.0-windows10.0.19041.0`?** The `10.0.19041.0` floor is what surfaces the **WinRT MIDI projections** (`Windows.Devices.Midi`, `Windows.Devices.Enumeration`) that `NAudio.Midi`'s Windows leg compiles against, so a leg at that floor is needed to resolve its WinRT-carrying asset.

But that floor cannot be the *only* Windows leg. NuGet hands a consumer an asset only when the consumer's target platform *version* is greater than or equal to the asset's, and a project written as plain `netX.0-windows` — what the WinForms and WPF templates emit — infers `TargetPlatformVersion 7.0`. Such a project therefore cannot see a `-windows10.0.19041.0` asset at all: it silently falls back to the portable `net9.0` one, losing every Windows backend and getting an `AudioFileReader` compiled without its Media Foundation path. This shipped in 3.0.0 and was reported as #1407. The plain `net9.0-windows` leg exists to catch those consumers; it carries identical code and the same backend references, and the only thing it gives up is WinRT MIDI (`NAudio.Midi` resolves its portable leg at that TFM, so winmm `MidiIn`/`MidiOut` from `NAudio.WinMM` remain available).

The same three-leg shape applies to `NAudio.Extras`, which had inherited the identical two-leg TFM set and therefore the identical defect.

> **Rule of thumb.** Any package whose Windows leg carries a *version-floored* TFM needs a plain `-windows` companion leg unless every intended consumer is known to target that floor explicitly. A version floor on a leaf package silently narrows who can consume it, and NuGet reports nothing when it happens.

> **Correction (second round).** An earlier version of this note claimed the floor existed because *process-loopback capture* needed it. That's not accurate at the compile level: process loopback (`ActivateAudioInterfaceAsync` + `AUDIOCLIENT_PROCESS_LOOPBACK_PARAMS`) is COM interop whose structs are declared in-repo, so it builds against plain `net9.0`. Windows 10 2004 (build 19041) is its *runtime* requirement, now expressed precisely with `[SupportedOSPlatform("windows10.0.19041.0")]` on the process-loopback members rather than via the TFM. The genuine compile-time consumer of the versioned TFM was always WinRT MIDI. This is why `NAudio.Wasapi` could drop to plain `net9.0` once WinRT MIDI moved out (#1384).

Windows 10 2004 (May 2020) is the practical minimum for any NAudio 3 consumer anyway — anyone older can stay on NAudio 2.x. Note that this is a statement about the *runtime* floor, not about which TFM consumers write: plenty of projects running on Windows 11 still target plain `netX.0-windows`, which is precisely why the meta-package cannot assume the versioned TFM. The constituent packages `NAudio.WinMM`, `NAudio.Asio`, and `NAudio.WinForms` continue to target `net9.0-windows` themselves (they have no 19041 dependency), so they remain individually consumable when referenced directly without the meta-package; `NAudio.Wasapi` targets plain `net9.0`.
...
### 5. Move WinRT MIDI to `NAudio.Midi`; drop `NAudio.Wasapi` to `net9.0` — **Done (2026-07-09)**

Prompted by #1384 (cross-platform apps couldn't build against `NAudio.Wasapi` off-Windows without `EnableWindowsTargeting`). Investigation showed the versioned Windows TFM had exactly one genuine compile-time consumer — the WinRT MIDI shim (`Windows.Devices.Midi`); WASAPI/CoreAudio and Media Foundation are all COM interop + P/Invoke that compiles anywhere.

- `WinRTMidiIn` / `WinRTMidiOut` / `MidiMessageConverter` (already in namespace `NAudio.Midi`, zero WASAPI coupling) moved from `NAudio.Wasapi/Midi/WinRT/` to `NAudio.Midi/Midi/WinRT/`. `NAudio.Midi` became dual-target `net9.0;net9.0-windows10.0.19041.0`; the WinRT files compile only on the Windows leg (`Compile Remove` on the portable leg). `EnableWindowsTargeting=true` keeps the source building on Linux/macOS (e.g. `NAudio.Core.Tests` project-references it) — verified: `NAudio.Core.Tests` builds and its 1461 cross-platform tests pass on Linux with no flag.
- `NAudio.Wasapi` dropped to plain `net9.0`, relying on its existing assembly-level `[SupportedOSPlatform("windows")]`; removed its now-unused `NAudio.Midi` `ProjectReference`. Verified: a cross-platform app referencing `NAudio.Wasapi` builds on Linux with no `EnableWindowsTargeting`.
- Meta-package: `NAudio.Wasapi` joined the ordinary Windows-leg reference group (no longer imposes the 19041 floor); the Windows leg stays `net9.0-windows10.0.19041.0` because `NAudio.Midi`'s WinRT asset needs it; the `WASAPI` compile constant now keys on the Windows leg.
- Restored the 19041 signal precisely: `[SupportedOSPlatform("windows10.0.19041.0")]` on `AudioClient.ActivateProcessLoopbackAsync`, `WasapiRecorderBuilder.WithProcessLoopback`, and internal `WasapiRecorder.CreateProcessLoopbackAsync`, with a `Debug.Assert(OperatingSystem.IsWindowsVersionAtLeast(10,0,19041))` guard at the shared `BuildAsync` dispatch. Builds clean under `TreatWarningsAsErrors`.

Docs updated: this file (design principles, the Wasapi/Midi rows, the meta-package "why 19041" note — including a correction of the earlier process-loopback rationale), `MODERNIZATION.md` (TFM table + platform-requirements row), and package READMEs. `RELEASE_NOTES.md` gained three bullets.
...
- **`AudioMediaSubtypes` namespace cleanup — done before 3.0.0.** `NAudio.Core/Wave/WaveFormats/AudioMediaSubtypes.cs` used to declare namespace `NAudio.Dmo`, so after the split both `NAudio.Core` and `NAudio.Dmo` contributed to it — and cross-platform callers needed `using NAudio.Dmo;` to name subtype GUIDs on Linux without the DMO package installed. The GUIDs aren't DMO-specific (they're cross-API media subtype GUIDs from `wmcodecdsp.h` / `mediaobj.h`), so the type moved to `NAudio.Wave`, beside its main consumer `WaveFormatExtensible` and matching its directory. `NAudio.MediaTypes` was considered and rejected as a namespace nobody would think to import. This is a public-API namespace change, so it landed in the major release rather than waiting for 4.0.
