...
You can also use `WaveFormatConversionStream` which is an ACM based resampler that has been in NAudio since the beginning. It resamples 16 bit audio only and you can't change the channel count at the same time. It requires a `WaveStream` as input. Here's it being used to resample an MP3 file:
