...
#### Phase 2f: Callback interfaces + IsAotCompatible
...
- All nine `Marshal.GetComInterfaceForObject` sites in `CoreAudioApi/` migrated to `ComActivation.ComWrappers.GetOrCreateComInterfaceForObject(callback, CreateComInterfaceFlags.None)`, each in `try/finally` so the CCW IUnknown ref is released even if `Register`/`Unregister` throws: `AudioEndpointVolume.cs` (register + Dispose), `AudioSessionControl.cs` (Dispose, RegisterEventClient, UnRegisterEventClient), `AudioSessionManager.cs` (RefreshSessions + UnregisterNotifications), `MMDeviceEnumerator.cs` (RegisterEndpointNotificationCallback + UnregisterEndpointNotificationCallback).
- `ActivateAudioInterfaceAsync` P/Invoke refactored from `[DllImport]` (with typed COM interface argument and `PreserveSig=false`) to `[LibraryImport]` with raw `IntPtr` and `int` HRESULT return. The single caller (`AudioClient.ActivateAsync`) now `Marshal.QueryInterface`s the multi-vtable CCW for `IID_IActivateAudioInterfaceCompletionHandler` before passing the pointer to native — the IUnknown CCW pointer points at a different vtable and would crash native (STATUS_STACK_BUFFER_OVERRUN, per Phase 2e hazards).
- `<IsAotCompatible>true</IsAotCompatible>` is now on. Trimmed publish (AotSmoke harness with `BuiltInComInteropSupport=false`) reports zero IL2026/IL3050 warnings against `NAudio.Wasapi` and runs end-to-end. NAudioTests: 1179 / 14 skipped / 0 failed.
...
- `IAudioSessionEvents` parameter shapes changed: `ref Guid eventContext` → `in Guid eventContext`, `[MarshalAs(UnmanagedType.Bool)] bool isMuted` → `int isMuted` on `OnSimpleVolumeChanged`. Direct subclasses of `AudioSessionEventsCallback` would need to update overrides; the public-facing `IAudioSessionEventsHandler` is unchanged.
