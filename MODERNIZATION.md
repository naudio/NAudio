...
- **Verification:**
  ...
  - `dotnet publish -p:PublishTrimmed=true -p:BuiltInComInteropSupport=false` against AotSmoke: runs end-to-end, zero IL2026/IL3050 against `NAudio.Wasapi`.
  - **`PublishAot=true` + `BuiltInComInteropSupport=false` against AotSmoke** (RCW + CCW directions): 7 endpoints enumerated with full property reads (VT_LPWSTR / VT_UI4 / VT_BLOB), `IMMNotificationClient` registers HR=0, four master-volume changes drove four `OnVolumeNotification` callbacks, clean exit. Strongest signal Phase 2f can have under genuine NativeAOT whole-program analysis.
...
- `Tools/AotSmoke` extended to exercise CCW direction in addition to RCW direction. Previously only validated property reads; now also registers `IMMNotificationClient` + drives `OnVolumeNotification` under `PublishAot=true`.
