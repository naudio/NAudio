...
{
    internal sealed class MFByteStreamFromStream : IMFByteStream
    {
        private Stream stream;
        private uint queue_token;
        private long wrapper_init_pos;
        ...
        public int IsEndOfStream(out BOOL pfEndOfStream)
        {
            try
            {
                pfEndOfStream = stream.Position < stream.Length ? BOOL.FALSE : BOOL.TRUE;
            }
            catch (NotSupportedException)
            {
                pfEndOfStream = BOOL.FALSE;
                return CommonHResults.E_FAIL;
            }
            catch (System.IO.IOException)
            {
        ...
        public unsafe int Write(nint pb, int cb, out int pcbWritten)
        {
            try
            {
                using (var temp = System.Buffers.ArrayPool<byte>.Shared.RentByContext(cb))
                {
                    Unsafe.CopyBlockUnaligned(ref temp.GetPinnableReference(), ref Unsafe.AsRef<byte>(pb.ToPointer()), (uint)cb);
                    stream.Write(temp.Buffer.AsSpan(0, pcbWritten = cb));
                }
                return CommonHResults.S_OK;
            }
            catch (System.IO.IOException)
        ...
        public unsafe int Read(nint pb, int cb, out int pcbRead)
        {
            try
            {
                using (var temp = System.Buffers.ArrayPool<byte>.Shared.RentByContext(cb))
                {
                    int read = stream.Read(temp.Buffer);
                    if (read == 0)
                    {
                        pcbRead = 0;
                    }
                    ...
                    {
                        Unsafe.CopyBlockUnaligned(ref Unsafe.AsRef<byte>(pb.ToPointer()), ref temp.GetPinnableReference(), (uint)read);
                        pcbRead = read;
                    }
                }
                return CommonHResults.S_OK;
        ...
        public unsafe int EndRead(nint pResult, out int pcbRead)
        {
            ...
            try
            {
                ...
                var async_br = stream.Read(d.AllocatedDotNetBuffer, 0, d.DataSize);
                fixed (byte* p = d.AllocatedDotNetBuffer)
                {
                    Unsafe.CopyBlockUnaligned(d.NativePointer.ToPointer(), p, (uint)async_br);
                }
                pcbRead = async_br;
                return CommonHResults.S_OK;
            }
