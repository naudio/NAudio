...
namespace NAudio.MediaFoundation
{
	public class MediaFoundationProvider : IWaveProvider
	{
		...
		public MediaFoundationProvider(string url)
        {
			...
			resolver.CreateObjectFromURL(url, SourceResolverFlags.MF_RESOLUTION_MEDIASOURCE|
				SourceResolverFlags.MF_RESOLUTION_CONTENT_DOES_NOT_HAVE_TO_MATCH_EXTENSION_OR_MIME_TYPE, null, out _, out object _source);
			resolver.CreateObjectFromURL(url, SourceResolverFlags.MF_RESOLUTION_BYTESTREAM|
				SourceResolverFlags.MF_RESOLUTION_CONTENT_DOES_NOT_HAVE_TO_MATCH_EXTENSION_OR_MIME_TYPE, null, out _, out object _stream);
			...
			for (uint i = 0; i < sdcount; i++)
			{
				...
				if (streamtype == MediaTypes.MFMediaType_Audio)
				{
					...
					mediaType.GetUINT32(MediaFoundationAttributes.MF_MT_AUDIO_SAMPLES_PER_SECOND, out int rate);
