...
sealed class WinMmDecodeMp3Test : IConsoleTest
{
    ...
    public MenuPath? MenuLocation =>
        new("WinMM (Windows Multimedia)", "Decode MP3 (AcmMp3FrameDecompressor)", Group: "ACM Compression", Order: 0);
    ...
    public TestResult Run(TestContext ctx)
    {
        ...
        if (ctx.Interactive)
            AnsiConsole.Status().Spinner(Spinner.Known.Dots).Start("Decoding MP3 via ACM...", _ => DoDecode());
        else
            DoDecode();
        ...
        return TestResult.Pass(
            $"Decoded {totalBytesDecoded:N0} PCM bytes",
