 ...
 * **Effects:** `EffectChain` can now be edited while it plays — new `Insert`, `RemoveAt`, and `Move` methods (alongside `Add`) publish a new chain atomically, so adding, removing, or reordering effects takes effect on the next block without resetting the other effects' state. Editing from one thread while another calls `Read` is safe and `Read` stays lock-free
 * **Effects:** added `EffectChain.Reset()`, which resets every effect in the chain (clearing delay lines, filter history, reverb tails) — useful when reusing a chain on a new signal, e.g. after a seek
