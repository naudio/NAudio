 ...
 * **WaveFormatExtensible** — a new constructor lets you set the SubFormat, valid-bits-per-sample and channel mask explicitly (e.g. 32-bit integer PCM, or 24 valid bits in a 32-bit container), and `ValidBitsPerSample` / `ChannelMask` are now readable; a new `[Flags] Speakers` enum names the standard speaker positions for building channel masks (#1325)
