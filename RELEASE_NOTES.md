 ...
 * **Effects — delay & modulation:** `DelayEffect` (tempo-syncable with read-only `EffectiveDelayMilliseconds`, feedback damping, ping-pong), `ChorusEffect`, `FlangerEffect`, `PhaserEffect` and `TremoloEffect` (with auto-pan)
