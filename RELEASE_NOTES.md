 ...
 * **SFZ Tier-1 finish:** the sampler now honours SFZ `trigger` (release samples play on note-off; `first`/`legato` select on held notes), `loop_mode=one_shot` (plays through note-off), directional `off_by` choke groups, and high/band-pass `fil_type` (in addition to low-pass). New state-preserving `BiQuadFilter.UpdateHighPassFilter`/`UpdateBandPassFilter` (mirroring `UpdateLowPassFilter`) back the modulatable high/band-pass voice filters
