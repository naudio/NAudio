...
- Fixed `AudioFileReader` wrapping WAV files in an ACM conversion stream unnecessarily: a `WaveFormatExtensible` header carrying ordinary PCM or IEEE float samples reports its encoding as `Extensible`, so it failed the PCM/IEEE check and was routed through `WaveFormatConversionStream`. The format is now reduced with `AsStandardWaveFormat()` first, leaving only genuinely compressed WAVs to be converted
- Improved the WPF demo's Audio Playback and Graphic EQ panels: both gained a volume slider and a seekable position slider with a time readout alongside the transport buttons, the EQ sliders now label their centre frequency and gain scale in dB, and the live waveform's options panel hides the settings that don't apply to the selected render style
- Fixed WPF demo sliders jumping straight to minimum or maximum when clicking the track — affecting the Realtime Effects parameter and Mix sliders and the Media Foundation Playback position bar. `Slider`'s default `SmallChange`/`LargeChange` are absolute (0.1 and 1), so on a 0..1 range a single click crossed the whole range. Those sliders now seek to the clicked point, and keyboard steps across the Realtime Effects and Sample Editor panels scale to each parameter's range instead of using the absolute defaults
- Rewrote the WPF demo's WASAPI capture panel as **WASAPI Recorder**, built on `WasapiRecorder`/`WasapiRecorderBuilder` instead of the obsolete `WasapiCapture`. It now exposes the builder options worth sanity-checking — capture device, loopback, default-device stream routing and per-process loopback, share mode, event/polling sync, buffer length, low latency (with the "required" variant), raw mode, communications mode, MMCSS priority and an optional explicit capture format — reports back what was actually negotiated (opened format, effective latency, whether IAudioClient3 low latency engaged and, when it didn't, why), and adds a Monitor button that runs the recorder and meters the input without writing to disk. The meter is computed from the captured audio itself rather than the endpoint's `AudioMeterInformation`, so it works for loopback and process capture too; the record-level slider (which just moved the endpoint's master volume) has been dropped
- Added `MMDeviceEnumerator.CreateNotificationClient()` returning an `MMDeviceNotificationClient` — an event-based way to observe endpoint changes (`DeviceStateChanged`, `DeviceAdded`, `DeviceRemoved`, `DefaultDeviceChanged`, `PropertyValueChanged`) that wraps the raw COM callback internally, so callers no longer need a `[GeneratedComClass]`, unsafe code, or manual CCW-lifetime management. Events marshal to the captured `SynchronizationContext` by default
- **Breaking:** made the low-level `IMMNotificationClient` interface and `MMDeviceEnumerator.RegisterEndpointNotificationCallback` / `UnregisterEndpointNotificationCallback` methods internal — use `CreateNotificationClient()` and its events instead. This hides the COM callback boundary in line with the rest of the library
- Fixed an access violation (`0xC0000005` in `ComInterfaceDispatch.GetInstance`) that could crash the process when an endpoint notification (e.g. a master-volume change) fired after a client had been registered via `MMDeviceEnumerator.RegisterEndpointNotificationCallback`. Windows does not AddRef the client, so the CCW was being collected; the enumerator now retains the reference for the registration's lifetime and releases it on unregister/dispose
...
- Added optional `contentType` (MIME) and `originName` (file name/URL) hint parameters to the `StreamMediaFoundationReader` constructor, exposed to Media Foundation as `MF_BYTESTREAM_CONTENT_TYPE` / `MF_BYTESTREAM_ORIGIN_NAME` so callers who know the format can steer byte-stream handler resolution instead of relying on content sniffing; supplying either hint skips the sniffer (#952)
...
- Removed redundant per-channel input copies in the `AsioDevice` duplex and recording capture paths. `AsioProcessBuffers.GetInput` / `AsioAudioCapturedEventArgs.GetChannel` now alias the driver buffer directly when the native input format is already 32-bit float (`Float32LSB`), and for other formats the native→float conversion is deferred to first access, so channels read only via `RawInput` (or not read at all) are never converted. Behaviour and the callback-scoped span lifetime are unchanged (#1389)
 ...
 * **MIDI** — `NAudio.Midi`'s portable leg is now cross-platform; new WinRT `WinRTMidiIn` / `WinRTMidiOut` (in `NAudio.Midi`'s Windows build) and backend-agnostic `IMidiInput` / `IMidiOutput`; a new `IMidiInstrument` seam with `MidiFileSequence` / `SequencedMidiPlayer` / `OfflineMidiRenderer` / `LiveMidiInstrument` gives an end-to-end MIDI-file → audio pipeline that drives the sampler or a hosted VST 3 instrument. `MidiFile` now also reads RIFF-RMID (`.rmi`) files (#1236); `MidiFile.Export` gains a `Stream` overload for writing to `MemoryStream` or any other seekable stream without going through a temp file, thanks to @MaKiPL (#499)
 ...
 * **Live capture mixing** — new `NAudio.Extras` helpers `CaptureMixerInput` and `RealtimeCaptureMixer` capture and live-mix several sources with different sample rates/channel counts (e.g. microphone + system loopback) into one stream. Output is paced to the wall clock (anchored to the first captured sample) so independently-clocked sources stay locked to real time and loopback gaps are handled cleanly. See [Docs/MixMicrophoneAndSystemAudio.md](Docs/MixMicrophoneAndSystemAudio.md) (#761)
 * **WASAPI device identity** — `WasapiPlayer` and `WasapiRecorder` now expose `DeviceId` and `DeviceFriendlyName` for the device they were created on (including when the default endpoint was resolved for you), so you can log the active device or persist its ID to reopen the same physical endpoint later; both are null when following the default device with automatic stream routing (#681)
 ...
 * **macOS support** - A new wrappers library (is in pre-release stage for NAudio 3) was added wrapping macOS native API's for playback/recording, reading/writing files and resampling audio. Special thanks to @mdcdi1315 (#1325) for the work and the tests. For more information, see the [design](Docs/Architecture/MacOSWrappersDesign.md) doc that describes the decisions that helped shape the wrappers. (#1398)
 ...
 * `ImaAdpcmWaveFormat` was removed — it was a non-functional "work in progress" stub (zero block align / samples-per-block, no serialization of its extension field) that was used nowhere; the `WaveFormatEncoding.ImaAdpcm`/`DviAdpcm` constants remain, and you can declare a custom `WaveFormat` subclass if you need an IMA ADPCM header
 ...
 * `WasapiRecorder.DataAvailable`: silent packets (commonly the first packet after `StartRecording`, while capture ramps up) no longer leak the raw, uninitialised WASAPI buffer — whose contents are undefined and could be stale audio heard as a short tone/noise burst at the start of a recording — and now deliver real silence, matching the legacy `WasapiCapture`
 ...
 * `FastFourierTransform.FFT`: fixed drifting high-frequency bins at large FFT sizes — the twiddle-factor recurrence is now carried in double precision so it no longer accumulates single-precision rounding error across stages; the butterfly arithmetic stays in float so throughput is unchanged (#520)
