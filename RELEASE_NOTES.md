 ...
 * **Sequencing:** new `NAudio.Sequencing` namespace in `NAudio.Core` with portable primitives for scheduling musical events — `ITempoMap` (with `LiveTempoMap` and `StaticTempoMap` implementations), `TimeSignatureMap`, `Transport`, `EventTimeline<T>`, `SwingTransform`, and a `SequencedSampleProvider<T>` audio bridge that dispatches events with sample-accurate offsets. See `Docs/Architecture/Sequencing.md`
 ...
 * **Drum machine demo:** rebuilt on top of the new `NAudio.Sequencing` primitives, with a swing knob, a "Render to WAV" command exercising the offline sample-driven path, and a "Use legacy PatternSequencer" toggle to A/B the two engines
