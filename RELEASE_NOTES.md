 ...
 * `WasapiOut` / `WasapiPlayer`: fixed a `RaceOnRCWCleanup` crash where a caller polling `PlaybackState` could dispose the audio client while the playback thread was still resetting it on the natural end-of-stream path — `Stop()`/`Dispose()` now reliably wait for the playback thread, and `WasapiPlayer` publishes `PlaybackState.Stopped` only after the device is reset (#970)
 * `WasapiPlayer`: `Stop()` no longer blocks the caller for up to the buffer latency — it now requests the stop and returns immediately, with completion signalled by `PlaybackStopped`; a new `StopAsync()` awaits until the device has actually been released (#1248)
