...
"### X.Y.Z (date)" at release time. See CLAUDE.md and
 ...
 * **Breaking:** `WaveFormat.FromFormatChunk` (and so `WaveFileReader.WaveFormat`, `AudioFileReader` and anything else reading a WAV fmt chunk) now returns the most specific format type instead of always returning a `WaveFormatExtraData` — a `WaveFormatExtensible`, `AdpcmWaveFormat`, `Gsm610WaveFormat` or `Mp3WaveFormat` where the encoding and extra data allow, a plain `WaveFormat` where there is no extra data, and a `WaveFormatExtraData` only for an encoding with no dedicated subclass. This is the same choice `WaveFormat.MarshalFromPtr` makes, so a format read from a file and the same format read from a native pointer now come back as the same type. Code casting a reader's `WaveFormat` to `WaveFormatExtraData`, or unpacking a `SubFormat` GUID out of its `ExtraData`, should pattern-match on `WaveFormatExtensible` (or call `AsStandardWaveFormat()`) instead
 * `WaveFormat.MarshalFromPtr` now decodes a WAVE_FORMAT_MPEGLAYER3 block into an `Mp3WaveFormat` rather than a `WaveFormatExtraData` when its cbSize accounts for the 12 MPEGLAYER3WAVEFORMAT bytes
 * A `WaveFormat` subclass decoded from a format block now reports the `ExtraSize` its `Serialize` actually writes, so a block whose cbSize over-reported the extension (drivers pad it, and a fmt chunk can be padded too) no longer round-trips into a fmt chunk shorter than the cbSize it advertises
