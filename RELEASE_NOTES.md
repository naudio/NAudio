 ...
 * **SoundFont modulators:** `ModulatorType` now exposes its decoded fields (`Polarity`, `Direction`, `IsMidiContinuousController`, `ControllerSource`, `SourceType`, `MidiContinuousControllerNumber`) instead of keeping them private — needed by the forthcoming sampler modulator engine
