 ...
 * **New `NAudio.Sampler` package:** `SoundFontSampler` is a polyphonic, cross-platform software sampler that plays a SoundFont through MIDI events and renders 32-bit float stereo as an `ISampleProvider`. v1 voice engine: pitch from root-key/tuning, the three SF2 loop modes, DAHDSR amplitude envelopes, a static per-voice low-pass filter, velocity/attenuation gain, equal-power pan, voice stealing, exclusive-class (choke) groups, and channel state (program/bank select, pitch-bend, sustain pedal, volume/expression). The SoundFont modulator engine (LFOs, mod envelope, the default modulators) follows in a later step (see `Docs/Architecture/SamplerDesign.md`)
