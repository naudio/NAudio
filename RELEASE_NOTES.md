...
- Removed a redundant per-channel input copy in the `AsioDevice` duplex and recording capture paths when the driver's native input format is already 32-bit float (`Float32LSB`): `AsioProcessBuffers.GetInput` / `AsioAudioCapturedEventArgs.GetChannel` now alias the driver buffer directly instead of returning a copy staged through an intermediate float buffer. Behaviour and the callback-scoped span lifetime are unchanged (#1389)
