 ...
 * **Latency reporting:** new `IWaveLatency` interface exposes `AverageLatency` and `CurrentLatency` for downstream A/V sync. Implemented on `WaveOut`, `WaveOutWindow`, `WasapiOut`, `WasapiPlayer`, `AsioOut`, and `AlsaOut` (#601)
