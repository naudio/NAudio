...
internal sealed class WinMmConvertWithStreamTest : IConsoleTest
{
    public string Id => "WinMm.ConvertWithStream";
    public string Description => "Convert a WAV file's sample rate via WaveFormatConversionStream (ACM)";
    public MenuPath? MenuLocation =>
        new("WinMM (Windows Multimedia)", "Convert format stream (WaveFormatConversionStream)",
            Group: "ACM Compression", Order: 2);
    ...
    public TestResult Run(TestContext ctx)
    {
        ...
        void DoConvert()
        {
            ...
            while (!ctx.Cancellation.IsCancellationRequested
                   && (bytesRead = conversionStream.Read(buffer, 0, buffer.Length)) > 0)
            {
        ...
        if (ctx.Interactive)
            AnsiConsole.Status().Spinner(Spinner.Known.Dots).Start("Converting format via ACM stream...", _ => DoConvert());
        else
            DoConvert();
