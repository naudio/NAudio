...
namespace NAudio.Dsp
{
    ...
    *****************************************************************************/
    ...
    public class SmbPitchShifter
    {
        ...
        /// <summary>
        /// Pitch Shift (array overload — forwards to the <see cref="Span{T}"/> version)
        /// </summary>
        public void PitchShift(float pitchShift, long numSampsToProcess, long fftFrameSize,
            long osamp, float sampleRate, float[] indata)
        {
            PitchShift(pitchShift, numSampsToProcess, fftFrameSize, osamp, sampleRate,
                indata.AsSpan(0, (int)numSampsToProcess));
        }

        /// <summary>
        /// Pitch Shift — processes <paramref name="indata"/> in place.
        /// </summary>
        public void PitchShift(float pitchShift, long numSampsToProcess, long fftFrameSize,
            long osamp, float sampleRate, Span<float> indata)
        {
            ...
            int nSamps = (int)numSampsToProcess;
            int nFft = (int)fftFrameSize;
            int nOsamp = (int)osamp;
            ...
            for (i = 0; i < nSamps; i++)
            {
                ...
                if (gRover >= nFft)
                {
                    ...
                    /* ***************** ANALYSIS ******************* */
                    /* do transform */
                    ShortTimeFourierTransform(gFFTworksp.AsSpan(), nFft, -1);
                    ...
                    for (k = nFft + 2; k < 2 * nFft; k++) gFFTworksp[k] = 0.0F;
        ...
        public void ShortTimeFourierTransform(float[] fftBuffer, long fftFrameSize, long sign)
            => ShortTimeFourierTransform(fftBuffer.AsSpan(), fftFrameSize, sign);

        /// <summary>
        /// Short Time Fourier Transform (span overload — operates in place)
        /// </summary>
        public void ShortTimeFourierTransform(Span<float> fftBuffer, long fftFrameSize, long sign)
        {
            ...
            for (i = 2; i < 2 * nFft - 2; i += 2)
            {
                for (bitm = 2, j = 0; bitm < 2 * nFft; bitm <<= 1)
                {
                    if ((i & bitm) != 0) j++;
                    j <<= 1;
                }
            ...
            int max = (int)(Math.Log(nFft) / Math.Log(2.0) + .5);
