                ...
                for (uint nch = 0U; nch < bufferChannelCount; nch++, J++)
                {
                    ...
                    for (uint F = 0U, BI = J * bytesPerSampleDerivedFromBits; F < nFramesToGet; F++, BI += targetBufferStride)
                    {
                        Unsafe.CopyBlockUnaligned(
                            ...
                            ref *((byte*)buffer.mData.ToPointer() + (F * bytesPerFrame * bufferChannelCount)), // Equivalent to: ref Unsafe.Add(ref Unsafe.AsRef<byte>(buffer.mData.ToPointer()), (int)(F * bytesPerFrame * bufferChannelCount))
